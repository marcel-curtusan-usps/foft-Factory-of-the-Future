...
namespace Factory_of_the_Future
{
    public class SourceDataProcess : IDisposable
    {
        ...
        internal static void Start_processor(object state)
        {
            ...
            try
            {
                if (Global.AppSettings.ContainsKey("FACILITY_NASS_CODE"))
                {
                    if (!string.IsNullOrEmpty((string)Global.AppSettings.Property("FACILITY_NASS_CODE").Value))
                    {
                        //string nasscode = (string)Global.AppSettings.Property("FACILITY_NASS_CODE").Value;
                        ...
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty((string)Global.AppSettings.Property("FACILITY_TIMEZONE").Value))
                            {
                                ...
                                {
                                    if (Connection.SERVER.Property("URL").Value.ToString().StartsWith("http"))
                                    {
                                       // webClient.Headers.Add(HttpRequestHeader.ContentType, "application/json");
                                        ...
                                        if (Connection.SERVER.Property("CONNECTION_NAME").Value.ToString().ToUpper().StartsWith("MPEWatch".ToUpper()))
                                        {
                                            if (!string.IsNullOrEmpty((string)Global.AppSettings.Property("MPE_WATCH_ID").Value))
                                            {
                                                ...
                                                DateTime dtNow = TimeZoneInfo.ConvertTime(DateTime.Now, TimeZoneInfo.FindSystemTimeZoneById(windowsTimeZoneId));
                                                int currentHour = dtNow.Hour;
                                                DateTime modsDate = DateTime.Now;
                                                ...
                                                switch (Connection.SERVER.Property("MESSAGE_TYPE").Value.ToString().ToUpper())
                                                {
                                                    ...
                                                    default:
                                                        ...
                                                        endDate = TimeZoneInfo.ConvertTime(endDate, TimeZoneInfo.FindSystemTimeZoneById(windowsTimeZoneId));
                                        ...
                                        {
                                            string start_time = string.Concat(DateTime.Now.AddHours(-2).ToString("yyyy-MM-dd'T'HH:"), "00:00");
                                            string end_time = DateTime.Now.AddHours(+10).ToString("yyyy-MM-dd'T'HH:mm:ss");
                                            ...
                                            if (!string.IsNullOrEmpty(data_source))
                                            {
                                                ...
                                                if (!string.IsNullOrEmpty(p2p_siteid))
                                                {
                                                    if (data_source.ToUpper().StartsWith("P2PBySite".ToUpper()))
                                                    {
                                                       ...
                                                       // webClient.Headers.Add(HttpRequestHeader.Authorization, (string)SourceData_list.SERVER.Property("OUTGOING_APIKEY").Value);
                                                    }
                                        ...
                                        else if (Connection.SERVER.Property("CONNECTION_NAME").Value.ToString().ToUpper().StartsWith("CTS".ToUpper()))
                                        {
                                            ...
                                            string nass = (string)Global.AppSettings.Property("FACILITY_NASS_CODE").Value;
                                            if (!string.IsNullOrEmpty(nass))
                                            {
                                                string key = (string)Connection.SERVER.Property("OUTGOING_APIKEY").Value;
                                                if (!string.IsNullOrEmpty(key))
                                                {
                                                    string data_source = (string)Connection.SERVER.Property("MESSAGE_TYPE").Value;
                                                    if (!string.IsNullOrEmpty(data_source))
                                                    {
                                                        formatUrl = string.Format((string)Connection.SERVER.Property("URL").Value, data_source, key, nass);
                                                    }
                                                }
                                            }
                                        }
