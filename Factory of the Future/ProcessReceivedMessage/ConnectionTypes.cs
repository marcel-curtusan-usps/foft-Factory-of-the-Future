    ...
    internal class ConnectionTypes
    {
        ...
        internal object Edit(string data)
        {
            ...
            try
            {
                ...
                if (fileUpdate)
                {
                    new FileIO().Write(string.Concat(AppParameters.CodeBase.Parent.FullName.ToString(), AppParameters.Appsetting), "ConnectionType.json", JsonConvert.SerializeObject(AppParameters.AppSettings, Formatting.Indented));
                }
                return AppParameters.connectionTypes;
            }
            catch (Exception e)
            {
