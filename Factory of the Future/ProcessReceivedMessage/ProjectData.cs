...
namespace Factory_of_the_Future
{
    internal class ProjectData : IDisposable
    {
                    ...
                    if (tempData.HasValues)
                    {
                        ...
                        else
                        {
                            for (int i = 0; i < tempData.Count(); i++)
                            {
                                ...
                                //if (!AppParameters.CoordinateSystem.TryAdd(tempCoordinateSystem.Id, tempCoordinateSystem))
                                //{
                                //    new ErrorLogger().CustomLog("Unable to add CoordinateSystem " + tempCoordinateSystem.Id, string.Concat(AppParameters.AppSettings["APPLICATION_NAME"].ToString(), "_Applogs"));
                                //}
                                //    if (AppParameters.CoordinateSystem.Count > 0 && AppParameters.CoordinateSystem.ContainsKey(tempData[i]["id"].ToString()))
                                //{
                                //    if (AppParameters.CoordinateSystem.TryGetValue(tempData[i]["id"].ToString(), out CoordinateSystem updateCS))
                                //    {
                                //        //the background image
                                //        LoadBackgroundImage(tempData[i].SelectToken("backgroundImages"), updateCS.Id, tempData[i]["name"].ToString(), out saveToFile);
                                //        //this is for Zones
                                //        LoadZones(tempData[i].SelectToken("zones"), updateCS.Id, out saveToFile);
                                //        //this is for Locators
                                //        LoadLocators(tempData[i].SelectToken("locators"), updateCS.Id, out saveToFile);

                                //    }
                                //}
                                //else
                                //{
                                //    CoordinateSystem CSystem = tempData[i].ToObject<CoordinateSystem>();
                                //    //CoordinateSystem CSystem = new CoordinateSystem
                                //    //{
                                ...
                                //    {
                                //        //the background image
                                //        LoadBackgroundImage(tempData[i].SelectToken("backgroundImages"), CSystem.Id, CSystem.Name, out saveToFile);
                                //        //this is for Zones
                                //        LoadZones(tempData[i].SelectToken("zones"), CSystem.Id, out saveToFile);
                                //        //this is for Locators
                                //        LoadLocators(tempData[i].SelectToken("locators"), CSystem.Id, out saveToFile);
                                //    }
                                //    else
                                //    {
                                //        new ErrorLogger().CustomLog("Unable to add CoordinateSystem " + CSystem.Id, string.Concat((string)AppParameters.AppSettings.Property("APPLICATION_NAME").Value, "_Applogs"));
        ...
        //private void LoadBackgroundImage(JToken backgroundImages, string csid, string csname, out bool saveToFile)
        //{
        //    saveToFile = false;
        //    try
        //    {
        //        if (backgroundImages.HasValues && backgroundImages.Count() > 0)
        //        {
        //            foreach (JToken bgItem in backgroundImages.Children())
        //            {
